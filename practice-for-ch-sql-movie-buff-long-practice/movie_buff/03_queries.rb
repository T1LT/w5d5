def what_was_that_one_with(those_actors)
  ...
  # getting movies where any actors in here
  # ensure to get all the actors in the movie
  # exclude movies that didnt have those_actors
  # look at distinct movie names
  # x ^ y => !x v !y
...
def golden_age
  ...
  Movie.select("(yr / 10) * 10 as decade").group("decade").order("avg(score) desc").limit(1)
end
