...
def biggest_cast
  ...
  # actors).
  Movie.select(:id, :title).joins(:actors).group(:id).order("count(castings.movie_id) DESC").limit(3)
end
...
def directed_by_one_of(them)
  ...
  # select id, title from movies join actors on actors.id = movies.director_id
